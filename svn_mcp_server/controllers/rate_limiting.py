...
def get_request_limit():
    """
    Get request limit from system parameter `svn_mcp_server.request_limit`.
    ...
    try:
        limit = int(
            ...
            .sudo()
            .get_param("svn_mcp_server.request_limit", DEFAULT_REQUEST_LIMIT)
        )
...
def rate_limit(func):
    ...
    Uses the system parameter `svn_mcp_server.request_limit`
    ...
    def wrapper(*args, **kwargs):
        ...
        rate_limiting_enabled = (
            request.env["ir.config_parameter"]
            .sudo()
            .get_param("svn_mcp_server.enable_rate_limiting", "True")
            == "True"
        )
        ...
        if not check_rate_limit(user.id):
            ...
            request.env["scp.log"].sudo().log_rate_limit_exceeded(
                user_id=user.id,
...
def request_timeout(func: Callable) -> Callable:
    ...
    `svn_mcp_server.request_timeout` setting. Terminates request processing
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            timeout_seconds = int(
                request.env["ir.config_parameter"]
                .sudo()
                .get_param("svn_mcp_server.request_timeout", "30")
            )
