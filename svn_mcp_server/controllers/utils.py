...
def is_mcp_enabled() -> bool:
    """
    Check if MCP is globally enabled via `svn_mcp_server.enabled` system parameter.
    ...
    if _mcp_enabled_cache['timestamp'] is not None and (now - _mcp_enabled_cache['timestamp']).total_seconds() < CACHE_TTL_SECONDS:
        return _mcp_enabled_cache['value']

    # Get fresh value
    try:
        value = request.env['ir.config_parameter'].sudo().get_param('svn_mcp_server.enabled', 'False') == 'True'
...
def is_model_mcp_enabled(env: Environment, model_name: str) -> bool:
    ...
    Uses existing `scp.model.access.is_model_enabled()` method.
    Result is cached for 5 minutes to reduce database queries.
    ...
    # Check if cache is valid
    if model_name in _model_enabled_cache and (now - _model_enabled_cache[model_name]['timestamp']).total_seconds() < CACHE_TTL_SECONDS:
        return _model_enabled_cache[model_name]['value']

    # Get fresh value
    try:
        value = env['scp.model.access'].sudo().is_model_enabled(model_name)
...
def check_model_operation_allowed(env: Environment, model_name: str, operation: str) -> bool:
    ...
    Uses existing `scp.model.access.check_model_operation_enabled()` method.
    Result is cached for 5 minutes to reduce database queries.
    ...
    # Check if cache is valid
    if cache_key in _operation_enabled_cache and (now - _operation_enabled_cache[cache_key]['timestamp']).total_seconds() < CACHE_TTL_SECONDS:
        return _operation_enabled_cache[cache_key]['value']

    # Get fresh value
    try:
        value = env['scp.model.access'].sudo().check_model_operation_enabled(model_name, operation)
        ...
        message = f'Error checking if operation {operation} is allowed for model {model_name}: {e}'
...
def get_enabled_models(env: Environment) -> List[Dict[str, str]]:
    ...
    try:
        enabled_model_records = env['scp.model.access'].sudo().search([('active', '=', True)])
...
def get_model_allowed_operations(env: Environment, model_name: str) -> Dict[str, bool]:
    ...
    mcp_model_record = env['scp.model.access'].sudo().search([('model_name', '=', model_name), ('active', '=', True)], limit=1)
...
def check_mcp_access(env: Environment, model_name: str, method_name: str) -> bool:
    ...
    if not env['scp.model.access'].sudo().is_model_enabled(model_name):
        message = f"MCP: Access denied for XML-RPC method '{method_name}' on model '{model_name}'. Model not MCP enabled."
        _logger.info(message)
        return False

    operation = map_method_to_operation(method_name)
    if not operation:
        message = f"MCP: XML-RPC method '{method_name}' on model '{model_name}' has no defined MCP operation mapping. Access denied by default."
        _logger.warning(message)
        return False

    # Check if operation is allowed for this model
    operation_allowed = env['scp.model.access'].sudo().check_model_operation_enabled(model_name, operation)

    if not operation_allowed:
        message = f"MCP: Access denied for XML-RPC method '{method_name}' (operation '{operation}') on model '{model_name}'. Operation not allowed by MCP."
        _logger.info(message)
        return False
    ...
    message = f"MCP: Access granted for XML-RPC method '{method_name}' (operation '{operation}') on model '{model_name}'."
...
def get_mcp_server_version() -> str:
    ...
    try:
        manifest = modules.module.get_manifest('svn_mcp_server')
...
def get_system_info(env: Environment) -> Dict[str, Union[str, int]]:
    ...
    if is_mcp_enabled():  # Only count if MCP is globally on
        enabled_mcp_models_count = env['scp.model.access'].sudo().search_count([('active', '=', True)])
