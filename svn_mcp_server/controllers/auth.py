...
def get_user_from_api_key(api_key, log_success=True):
    """
    Get user from API key.

    :param api_key: The API key to validate
    :param log_success: Write an ``auth_success`` audit entry on success. The
        XML-RPC object dispatch resolves the key on *every* call, so it passes
        ``False`` to avoid flooding the log with one auth_success per read that
        merely duplicates the ``model_access`` entry already written. Failures
        are always logged. REST endpoints keep the default (once per request).
    ...
    try:
        ...
        if not user_id:
            # sudo: write to scp.log regardless of user permissions
