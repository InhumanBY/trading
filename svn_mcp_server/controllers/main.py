...
class McpAPIController(http.Controller):
    ...
    def get_model_access(self, model, **kwargs):
        ...
        ir_model = env['ir.model'].sudo().search([('model', '=', model_tech_name)], limit=1)
        if not ir_model:
        ...
        if not is_enabled:
            ...
            env['scp.log'].sudo().log_permission_denied(
                model_name=model_tech_name,
                operation='access',
                user_id=user.id if user else None,
                endpoint=request.httprequest.path,
        ...
        env['scp.log'].sudo().log_model_access(
            model_name=model_tech_name,
            operation='access',
            user_id=user.id if user else None,
            endpoint=request.httprequest.path,
