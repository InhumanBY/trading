...
class McpModelSelectionWizard(models.TransientModel):
    ...
    _name = 'scp.model.wizard'
    _description = 'MCP Model Selection Wizard'

    @api.model
    def _get_model_domain(self):
        # Get already enabled models
        enabled_model_ids = self.env['scp.model.access'].search([]).mapped('model_id.id')
        domain = [
            ('transient', '=', False),
            ('model', 'not like', 'ir.%'),
    ...
    allow_create = fields.Boolean(default=False, help='Allow create operations through MCP')
    ...
    def action_enable_models(self):
        ...
        for model in self.model_ids:
            ...
            existing = self.env['scp.model.access'].search([('model_id', '=', model.id)], limit=1)
            if not existing:
