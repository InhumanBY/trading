...
class TestMCPLogging(TransactionCase):
    def setUp(self):
        ...
        self.MCPLog = self.env["scp.log"].with_context(test_mcp_logging=True)
        self.test_user = create_test_user(
            self.env, "Test MCP User", "test_mcp_user", email="test_mcp@example.com"
        )
        # Enable logging
        self.env["ir.config_parameter"].sudo().set_param(
            "svn_mcp_server.enable_logging", "True"
        )
    ...
    def test_logging_disabled(self):
        ...
        # Disable logging
        self.env["ir.config_parameter"].sudo().set_param(
            "svn_mcp_server.enable_logging", "False"
        )
    ...
    def test_cleanup_with_config_parameter(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "svn_mcp_server.log_retention_days", "7"
        )
