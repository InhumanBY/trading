...
class TestMcpSecurity(common.TransactionCase):
    def setUp(self):
        ...
        self.mcp_user = create_test_user(
            ...
            group_ids=[(6, 0, [self.env.ref("svn_mcp_server.group_mcp_user").id])],
        )
    ...
    def test_mcp_admin_access(self):
        ...
        if not existing_model:
            ...
            self.assertFalse(
                self.env["scp.model.access"].search([("id", "=", new_model.id)]),
    ...
    def test_mcp_user_access(self):
        ...
        with self.assertRaises(
            ...
            self.env["scp.model.access"].with_user(self.mcp_user).create(
                {
    ...
    def test_settings_menu_access(self):
        ...
        try:
            menu_root = self.env.ref("svn_mcp_server.mcp_menu_root")
        except ValueError:
            # Fallback to search
        ...
        # Get the groups
        mcp_admin_group = self.env.ref("svn_mcp_server.group_mcp_admin")
        mcp_user_group = self.env.ref("svn_mcp_server.group_mcp_user")
