...
class TestMCPCommonController(HttpCase):
    def setUp(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("svn_mcp_server.enabled", "True")
    ...
    def test_common_controller_mcp_disabled(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param("svn_mcp_server.enabled", "False")
        # Clear the cache to ensure the new value is used
        utils.clear_mcp_caches()
    ...
    @mute_logger("odoo.addons.svn_mcp_server.controllers.xmlrpc")
    @patch("odoo.service.common.dispatch")
...
class TestMCPObjectController(HttpCase):
    ...
    @patch("odoo.addons.svn_mcp_server.controllers.utils.sanitize_model_name")
...
@tagged("much_unit", "post_install", "-at_install")
class TestClampResultLimit(HttpCase):
    """svn_mcp_server.max_records caps read page size (0 = unlimited default)."""

    def _patched_request(self, max_records):
        req = MagicMock()
        req.env.__getitem__.return_value.sudo.return_value.get_param.return_value = str(
            max_records
        )
        return req

    def _clamp(self, max_records, method, params):
        ctrl = MCPObjectController()
        with patch.object(api_module, "request", self._patched_request(max_records)):
            return ctrl._clamp_result_limit(method, params)

    def test_unlimited_default_no_change(self):
        params = ["db", 2, "k", "res.partner", "search_read", [[]], {"fields": ["id"]}]
        out = self._clamp(0, "search_read", params)
        self.assertNotIn("limit", out[6])

    def test_clamp_when_over(self):
        params = ["db", 2, "k", "res.partner", "search_read", [[]], {"limit": 100}]
        out = self._clamp(5, "search_read", params)
        self.assertEqual(out[6]["limit"], 5)

    def test_clamp_when_missing(self):
        params = ["db", 2, "k", "res.partner", "search_read", [[]], {}]
        out = self._clamp(5, "search_read", params)
        self.assertEqual(out[6]["limit"], 5)

    def test_appends_kwargs_when_absent(self):
        params = ["db", 2, "k", "res.partner", "search", [[]]]  # len 6, no kwargs
        out = self._clamp(5, "search", params)
        self.assertEqual(out[6]["limit"], 5)

    def test_within_limit_kept(self):
        params = ["db", 2, "k", "res.partner", "search_read", [[]], {"limit": 10}]
        out = self._clamp(50, "search_read", params)
        self.assertEqual(out[6]["limit"], 10)

    def test_non_read_untouched(self):
        params = ["db", 2, "k", "res.partner", "write", [[1], {"x": 1}], {}]
        out = self._clamp(5, "write", params)
