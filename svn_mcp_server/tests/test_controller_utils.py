...
class TestMcpUtils(common.TransactionCase):
    ...
    def test_is_mcp_enabled(self):
        ...
        with patch("odoo.addons.svn_mcp_server.controllers.utils.request", mock_request):
            # Test when enabled
            self.env["ir.config_parameter"].sudo().set_param(
                "svn_mcp_server.enabled", "True"
            )
            utils.clear_mcp_caches()
...
class TestAuthAndResponseUtils(common.TransactionCase):
    ...
    def test_get_user_from_api_key(self):
        ...
        with patch("odoo.addons.svn_mcp_server.controllers.auth.request", mock_request):
            # Valid key
    ...
    def test_success_response(self):
        ...
        with patch(
            "odoo.addons.svn_mcp_server.controllers.response_utils.request", mock_request
        ):
