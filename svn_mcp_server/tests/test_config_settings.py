...
class TestConfigSettings(TransactionCase):
    ...
    def test_create_and_save_settings(self):
        ...
        self.assertEqual(param_obj.get_param("svn_mcp_server.enabled"), "True")
        self.assertEqual(param_obj.get_param("svn_mcp_server.request_limit"), "100")
        self.assertEqual(param_obj.get_param("svn_mcp_server.request_timeout"), "45")
        self.assertEqual(param_obj.get_param("svn_mcp_server.enable_logging"), "True")
        self.assertEqual(param_obj.get_param("svn_mcp_server.enable_rate_limiting"), "True")
    ...
    def test_default_values(self):
        ...
        param_obj = self.env["ir.config_parameter"].sudo()
        params = [
            "svn_mcp_server.enabled",
            "svn_mcp_server.request_limit",
            "svn_mcp_server.request_timeout",
            "svn_mcp_server.enable_logging",
            "svn_mcp_server.enable_rate_limiting",
        ]
        for param in params:
    ...
    def test_unlimited_rate_limit_setting(self):
        ...
        param_obj = self.env["ir.config_parameter"].sudo()
        self.assertEqual(param_obj.get_param("svn_mcp_server.request_limit"), "0")

    def test_load_settings(self):
        """Test that settings are loaded from system parameters."""
        # Set some values in system parameters
        param_obj = self.env["ir.config_parameter"].sudo()
        param_obj.set_param("svn_mcp_server.enabled", "False")
        param_obj.set_param("svn_mcp_server.request_limit", "200")
        param_obj.set_param("svn_mcp_server.request_timeout", "60")
    ...
    def test_settings_ui_display(self):
        ...
        view_id = self.env.ref("svn_mcp_server.mcp_res_config_settings_view_form")
