...
class MCPLog(models.Model):
    ...
    # Additional metadata. The MCP session id lives in the bridge (streamable
    # transport) and is never forwarded down the XML-RPC call, so it cannot be
    # captured here; user_agent reflects the bridge/client that reached Odoo.
    ...
    def log_event(self, event_type, **kwargs):
        ...
        if self.env['ir.config_parameter'].sudo().get_param('svn_mcp_server.enable_logging', 'True') != 'True':
            return self.env['scp.log']
    ...
    def cleanup_old_logs(self, days=None):
        ...
        if days is None:
            ...
            days = int(self.env['ir.config_parameter'].sudo().get_param('svn_mcp_server.log_retention_days', '30'))
    ...
    def _register_hook(self):
        ...
        cron_vals = {
            ...
            'model_id': self.env['ir.model'].search([('model', '=', 'scp.log')]).id,
            'state': 'code',
    ...
    def action_open_records(self):
        """Open the records this entry touched, derived from model_name + record_ids.

        record_ids is a comma-separated Char (a call can touch many rows), so a
        Reference field would not fit; an act_window with an ``id in (...)``
        domain handles both single and multi-record cases. Parsing is defensive
        and records deleted since the call are simply absent from the result.
        """
        self.ensure_one()
        ids = [
            int(part)
            for part in (self.record_ids or '').split(',')
            if part.strip().lstrip('-').isdigit()
        ]
        if not self.model_name or self.model_name not in self.env or not ids:
            raise UserError(_('This log entry has no linked records to open.'))
        ...
        if len(ids) == 1:
            ...
            action.update(view_mode='list,form', domain=[('id', 'in', ids)])
        return action
